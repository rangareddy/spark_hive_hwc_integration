...
if [ ! -f "$hive_site_xml_file" ]; then
    echo "${hive_site_xml_file} does not exist on this host, or the current user $(whoami) does not have access to the files."
fi
...
echo "spark.sql.hive.hiveserver2.jdbc.url=jdbc:hive2://${hive_jdbc_url}"
